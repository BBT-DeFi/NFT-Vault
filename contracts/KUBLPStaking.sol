...
contract LPStakingV2 is Ownable {
    ...
    // Constructor will set the address of KUB/ETH LP token and address of KUB/ETH LP token staking NFT
    constructor(address _LPToken, address _KUBToken, address _StakingNFT, address _staking, address _rewardPool, uint _dailyReward) Ownable() public {
    ...
    // Function that will get balance of a KUB/ETH LP balance of a certain stake
    function getNFTBalance(uint _tokenId) public view returns(uint _amountStaked) {
        return NFTDetails[_tokenId]._LPDeposited;
    }

    // Function that will check if a KUB/ETH LP stake NFT is in circulation
    ...
    function pendingRewards(uint _NFT) public view returns(uint) {
        ...
        return nft._LPDeposited.mul(_accKUBPerShare).div(1e18).sub(nft._rewardDebt);
    }

    // Get total rewards for all of user's KUB/ETH LP nfts
    ...
    // Get total stake for all user's KUB/ETH LP nfts
    ...
    // Function that updates KUB/ETH LP pool
    ...
    // Function that lets user stake KUB/ETH LP
    function stakeLP(uint _amount) public {
        ...
        if(nft._LPDeposited > 0) {
            uint _pendingRewards = nft._LPDeposited.mul(accKUBPerShare).div(1e18).sub(nft._rewardDebt);
        ...
        nft._rewardDebt = nft._LPDeposited.mul(accKUBPerShare).div(1e18);
    ...
    // Function that will unstake every user's KUB/ETH LP stake NFT for user
