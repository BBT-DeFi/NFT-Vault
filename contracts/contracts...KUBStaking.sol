...
// File: contracts/KUBStakingV2.sol
...
contract KUBStakingV2 is Ownable {
    using SafeMath for uint256;
    ...
    IERC20 public KUBToken;
    IKUBStakingNFT public StakingNFT;
    address public rewardPool;
    address public staking;
    uint public dailyReward;
    uint public accKUBPerShare;
    uint public lastRewardBlock;
    uint public totalStaked;

    mapping(uint => NFT) public NFTDetails;

    // Constructor will set the address of KUB token and address of KUB staking NFT
    constructor(address _KUBToken, address _StakingNFT, address _staking, address _rewardPool, uint _dailyReward) Ownable() public {
        KUBToken = IERC20(_KUBToken);
        StakingNFT = IKUBStakingNFT(_StakingNFT);
        staking = _staking;
    ...
    // 6500 blocks in average day --- decimals * KUB balance of rewardPool / blocks / 10000 * dailyReward (in hundredths of %) = rewardPerBlock
    function getRewardPerBlock() public view returns(uint) {
        return KUBToken.balanceOf(rewardPool).mul(dailyReward).div(6500).div(10000);
    }
    ...
    // Function that will get balance of a KUB balance of a certain stake
    function getNFTBalance(uint _tokenId) public view returns(uint _amountStaked) {
        return NFTDetails[_tokenId]._KUBDeposited;
    }

    // Function that will check if a KUB stake NFT is in circulation
    ...
    function pendingRewards(uint _NFT) public view returns(uint) {
        ...
        uint256 _accKUBPerShare = accKUBPerShare;

        if (block.number > lastRewardBlock && totalStaked != 0) {
            uint256 blocksToReward = block.number.sub(lastRewardBlock);
            uint256 KUBReward = blocksToReward.mul(getRewardPerBlock());
            _accKUBPerShare = _accKUBPerShare.add(KUBReward.mul(1e18).div(totalStaked));
        }

        return nft._KUBDeposited.mul(_accKUBPerShare).div(1e18).sub(nft._rewardDebt);
    }

    // Get total rewards for all of user's KUB nfts
    ...
    // Get total stake for all user's KUB nfts
    ...
    // Function that updates KUB pool
    function updatePool() public {
        ...
        //Approve KUBReward here
        KUBToken.transferFrom(rewardPool, address(this), KUBReward);

        accKUBPerShare = accKUBPerShare.add(KUBReward.mul(1e18).div(totalStaked));
        lastRewardBlock = block.number;

        emit PoolUpdated(blocksToReward, KUBReward, now);
    }

    // Function that lets user stake KUB
    function stakeKUB(uint _amount) public {
        require(_amount > 0, "Can not stake 0 KUB");
        require(KUBToken.balanceOf(_msgSender()) >= _amount, "Do not have enough KUB to stake");
        ...
        if(nft._KUBDeposited > 0) {
            uint _pendingRewards = nft._KUBDeposited.mul(accKUBPerShare).div(1e18).sub(nft._rewardDebt);

            if(_pendingRewards > 0) {
                KUBToken.transfer(_msgSender(), _pendingRewards);
                emit RewardsClaimed(_msgSender(), _pendingRewards, StakingNFT.nftTokenId(_msgSender()), now);
            }
        }

        KUBToken.transferFrom(_msgSender(), address(this), _amount);
        nft._KUBDeposited = nft._KUBDeposited.add(_amount);
        totalStaked = totalStaked.add(_amount);

        nft._rewardDebt = nft._KUBDeposited.mul(accKUBPerShare).div(1e18);

        emit StakeCompleted(_msgSender(), _amount, StakingNFT.nftTokenId(_msgSender()), nft._KUBDeposited, now);
    ...
    function addStakeholderExternal2(address _stakeholder) external onlyPlatform() {
    ...
    // Function that will add KUB rewards to KUB staking NFT
    function compoundRewards(uint _tokenId) public {
        ...
        nft._KUBDeposited = nft._KUBDeposited.add(_pendingRewards);
        totalStaked = totalStaked.add(_pendingRewards);

        nft._rewardDebt = nft._KUBDeposited.mul(accKUBPerShare).div(1e18);

        emit RewardsCompounded(_msgSender(), _pendingRewards, _tokenId, nft._KUBDeposited, now);
    }
    ...
    // Function that lets user unstake KUB in system. 5% fee that gets redistributed back to reward pool
    function unstakeKUB(uint _tokenId) public {
        ...
        uint beingWithdrawn = nft._KUBDeposited;
        nft._KUBDeposited = 0;
    ...
    function incrementNFTValue (uint _tokenId, uint _amount) external onlyPlatform() {
        ...
        if(nft._KUBDeposited > 0) {
            ...
            if(_pendingRewards > 0) {
                KUBToken.transfer(StakingNFT.ownerOf(_tokenId), _pendingRewards);
                emit RewardsClaimed(StakingNFT.ownerOf(_tokenId), _pendingRewards, _tokenId, now);
            }
        }

        NFTDetails[_tokenId]._KUBDeposited =  NFTDetails[_tokenId]._KUBDeposited.add(_amount);
    ...
    function decrementNFTValue (uint _tokenId, uint _amount) external onlyPlatform() {
        ...
        NFTDetails[_tokenId]._KUBDeposited =  NFTDetails[_tokenId]._KUBDeposited.sub(_amount);
